...
| --- | --- | --- | --- |
| `apiVersion` _string_ | `placement.kubernetes-fleet.io/v1` | | |
| `kind` _string_ | `ClusterResourcePlacementStatusList` | | |
...
| `items` _[ClusterResourcePlacementStatus](#clusterresourceplacementstatus) array_ |  |  |  |
...
| `type` _[EnvelopeType](#envelopetype)_ | Type of the envelope object. |  | Enum: [ClusterResourceEnvelope ResourceEnvelope] <br />Optional: \{\} <br /> |
...
| `applyStrategy` _[ApplyStrategy](#applystrategy)_ | ApplyStrategy describes how to resolve the conflict if the resource to be placed already exists in the target cluster<br />and is owned by other appliers.<br />This field is a beta-level feature. |  | Optional: \{\} <br /> |
...
| --- | --- | --- | --- |
| `apiVersion` _string_ | `placement.kubernetes-fleet.io/v1` | | |
| `kind` _string_ | `ResourcePlacementList` | | |
| `metadata` _[ListMeta](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.34/#listmeta-v1-meta)_ | Refer to Kubernetes API documentation for fields of `metadata`. |  |  |
| `items` _[ResourcePlacement](#resourceplacement) array_ |  |  |  |
...
| Field | Description | Default | Validation |
| --- | --- | --- | --- |
| `observedCRPGeneration` _integer_ | ObservedCRPGeneration is the generation of the CRP which the scheduler uses to perform<br />the scheduling cycle and prepare the scheduling status. |  | Required: \{\} <br /> |
